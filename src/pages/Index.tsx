...
const Index = () => {
  ...
  const { data: searchResults, isLoading: isSearching } = useVehicles(
    searchActive ? {
      city: searchFilters.city,
      fromDate: searchFilters.fromDate,
      untilDate: searchFilters.untilDate,
      fromTime: searchFilters.fromTime,
      untilTime: searchFilters.untilTime,
    } : undefined
  );
  ...
  const handleSearch = (params: { city: string; fromDate?: string; untilDate?: string; fromTime: string; untilTime: string }) => {
  ...
  return (
    <div className="flex flex-col bg-background">
      ...
      <div className="md:hidden px-4 py-3">
        ...
        >
          ...
          <span className="text-sm text-muted-foreground">
            {searchActive && searchFilters.city ? searchFilters.city.split(",")[0] : "Inicie sua busca"}
          </span>
          {searchActive && (
            <span
              onClick={(e) => { e.stopPropagation(); clearSearch(); }}
              className="ml-auto w-6 h-6 rounded-full bg-muted flex items-center justify-center"
            >
      ...
      {mobileSearchOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-background flex flex-col">
          <div className="flex items-center justify-between px-4 py-3 border-b border-border">
            <span className="text-base font-semibold text-foreground">Buscar</span>
            <button onClick={() => setMobileSearchOpen(false)} className="w-8 h-8 rounded-full border border-border flex items-center justify-center">
              <X className="w-4 h-4" />
            </button>
          </div>
          <div className="flex-1 overflow-y-auto p-4">
            <TuroSearchBar onSearch={handleSearch} />
          </div>
        </div>
      )}
              ...
              <div>
                ...
                {searchFilters.city && (
                  <p className="text-sm text-muted-foreground mt-0.5">
                    {searchFilters.city.split(",")[0]}
                    {searchFilters.fromDate && searchFilters.untilDate && ` · ${searchFilters.fromDate} - ${searchFilters.untilDate}`}
                  </p>
                )}
              </div>
              <button
            ...
            <div className="mb-4">
              <p className="text-muted-foreground text-sm">
                {isSearching ? "Buscando..." : (
                  <>{searchResults?.length || 0} veículos encontrados</>
                )}
              </p>
            </div>
            ...
            {isSearching ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6">
                {searchResults.map((vehicle) => {
                  ...
                  if (searchFilters.fromDate) carLinkParams.set("from", searchFilters.fromDate);
                  if (searchFilters.untilDate) carLinkParams.set("until", searchFilters.untilDate);
                  if (searchFilters.fromTime) carLinkParams.set("fromTime", searchFilters.fromTime);
                  if (searchFilters.untilTime) carLinkParams.set("untilTime", searchFilters.untilTime);
                  return (
                    <VehicleCard key={vehicle.id} vehicle={vehicle} linkParams={carLinkParams.toString()} />
                  );
                })}
              </div>
            ) : (
      ...
      {/* Featured Cars - hidden when searching */}
      {!searchActive && (
        <section className="py-6 md:py-12 flex-1">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between mb-5">
      ...
      {/* App Driver Vehicles - hidden when searching */}
      {!searchActive && (
      ...
      {/* How it works - Desktop only, hidden when searching */}
      {!searchActive && (
      ...
      {/* CTA for Owners - Desktop only, hidden when searching */}
      {!searchActive && (
      ...
      {/* Benefits - Desktop only, hidden when searching */}
      {!searchActive && (
